            ...
            for track in self.tracker.tracker.tracks:
                ...
                if results.pose_landmarks:
                    ...
                    if len(self.person_buffers[track_id]) == self.buffer_size and violence_model:
                        ...
                        print(pred)
                        self.label = "NonViolence" if pred[0][0] > 0.5 else "Violence"
                        self.current_video_predictions.append(self.label)
                        self.predictions[track_id] = self.label
                        self.person_buffers[track_id].clear()
                ...
                # Draw label
                self.label = self.predictions.get(track_id, "P R O C E S S I N G")
                label_color = (255, 0, 255) if self.label == "P R O C E S S I N G" else (0, 255, 0) if self.label == "NonViolence" else (0, 0, 255)
                cv2.putText(og_frame, self.label, (x1, y1 + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, label_color, 2)

        return og_frame, self.label
